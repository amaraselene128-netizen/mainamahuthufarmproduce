-- =====================================================================
-- EGRATASKS Platform Overhaul — run this in the Supabase SQL editor.
...
begin
  ...
  insert into public.referrals(referrer_id, referred_id, generation)
    values (g1, new.user_id, 1) on conflict do nothing;
  select referred_by into g2 from public.profiles where user_id = g1;
  if g2 is not null then
    insert into public.referrals(referrer_id, referred_id, generation)
      values (g2, new.user_id, 2) on conflict do nothing;
    select referred_by into g3 from public.profiles where user_id = g2;
    if g3 is not null then
      insert into public.referrals(referrer_id, referred_id, generation)
        values (g3, new.user_id, 3) on conflict do nothing;
    end if;
  end if;
  return new;
end$$;
...
begin
  for r in select referrer_id, generation from public.referrals where referred_id = p_user loop
    ...
    if amt > 0 then
      ...
      update public.wallets set balance_cents = coalesce(balance_cents,0) + amt
...
begin
  ...
  update public.profiles set active_tier = p_tier where user_id = p_user;
